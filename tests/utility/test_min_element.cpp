...
TEST_CASE("dsa_min_element finds min element", "[dsa_min_element]")
{
    SECTION("min element in int array")
    {
        constexpr std::array<int, 5> arr{1, 5, 3, 4, 2};

        const size_t min_index = dsa_min_element(arr.data(), arr.size(), sizeof(int), compare_int);
    ...
    SECTION("min element in double array")
    {
        ...
        const size_t min_index = dsa_min_element(arr.data(), arr.size(), sizeof(double), compare_double);
    ...
    SECTION("min element in array of Points by x coordinate")
    {
        ...
        const size_t min_index = dsa_min_element(arr.data(), arr.size(), sizeof(Point), compare_point_x);
...
TEST_CASE("dsa_min_element with invalid input parameters", "[dsa_min_element]")
{
    SECTION("Handling empty input array")
    {
        const size_t count = 10;
        const size_t min_index = dsa_min_element(nullptr, count, sizeof(int), compare_int);
        REQUIRE(min_index == count);
    }

    SECTION("Handling array size equal 0")
    {
        constexpr std::array<double, 3> arr{1.0, 2.0, 3.0};
        const size_t min_index = dsa_min_element(arr.data(), 0, sizeof(double), compare_double);
        REQUIRE(min_index == 0);
    }
    ...
    {
        constexpr std::array<double, 3> arr{1.0, 2.0, 3.0};
        const size_t min_index = dsa_min_element(arr.data(), arr.size(), 0, compare_double);
        REQUIRE(min_index == arr.size());
    }

    SECTION("Handling invalid compare function")
    {
        std::array<int, 3> arr{3, 7, 10};
        const size_t min_index = dsa_min_element(arr.data(), arr.size(), sizeof(int), nullptr);
        REQUIRE(min_index == arr.size());
    }
}
